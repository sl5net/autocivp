var autociv_focus = {
	"gameList": function ()
	{
		let GUIobject = Engine.TryGetGUIObjectByName("gamesBox");
		GUIobject.blur();
		GUIobject.focus();
	},
	"chatInput" ()
	{
		let GUIobject = Engine.TryGetGUIObjectByName("chatInput");
		GUIobject.blur();
		GUIobject.focus();
...
autociv_patchApplyN("init", function (target, that, args)
{
	...
	Engine.TryGetGUIObjectByName("middlePanel").onTick = () =>
	{
		g_Time = Date.now();
		updateTimers()
	}
	...
	// React to GUI objects resize bars
	{
		...
		let gameInfo = Engine.TryGetGUIObjectByName("sgMapName")?.parent;
		if (gameInfo)
		{
...
function initChatFilterInput()
{
	...
	const chatText = Engine.TryGetGUIObjectByName("chatText")
	let originalList = []
...
{
	lobbyPageTitle = Engine.TryGetGUIObjectByName("lobbyPageTitle")
	nOfPlayers = 0
	nOfGames = 0
